...
for file in os.listdir(input_dir):
    if file.endswith(".ply"):
        ...
        # shift for depth (assuming unit bbox and cam dst 1.7)
        scale_node = tree.nodes.new('CompositorNodeMapValue') 
        scale_node.offset[0] = -1.2
        
        links = tree.links
        links.new(image_node.outputs[2], scale_node.inputs[0]) # depth -> shifted
        links.new(scale_node.outputs[0], comp_node.inputs[0]) # combined -> ouput    
